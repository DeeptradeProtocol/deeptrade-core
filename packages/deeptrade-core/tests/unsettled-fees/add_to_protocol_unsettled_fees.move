...
fun add_fee_for_new_coin_type_success() {
    ...
    scenario.next_tx(OWNER);
    {
        ...
        assert_eq!(fees_manager.get_protocol_unsettled_fee_balance<SUI>(), fee_amount);
...
fun aggregate_fees_for_existing_coin_type_success() {
    ...
    scenario.next_tx(OWNER);
    {
        ...
        // Add first fee
        fees_manager.add_to_protocol_unsettled_fees(fee_balance_1, ctx);
        assert_eq!(fees_manager.get_protocol_unsettled_fee_balance<SUI>(), fee_amount_1);

        // Add second fee
        fees_manager.add_to_protocol_unsettled_fees(fee_balance_2, ctx);

        // Verify total fee is aggregated
        assert_eq!(fees_manager.get_protocol_unsettled_fee_balance<SUI>(), expected_total_fee);
...
fun handle_multiple_distinct_coin_types_correctly() {
    ...
    scenario.next_tx(OWNER);
    {
        ...
        assert_eq!(fees_manager.has_protocol_unsettled_fee<SUI>(), false);
        assert_eq!(fees_manager.has_protocol_unsettled_fee<DEEP>(), false);

        // Add SUI fee
        fees_manager.add_to_protocol_unsettled_fees(sui_fee, ctx);

        // Add DEEP fee
        fees_manager.add_to_protocol_unsettled_fees(deep_fee, ctx);

        // Verify both fees exist and are stored correctly
        assert_eq!(fees_manager.has_protocol_unsettled_fee<SUI>(), true);
        assert_eq!(fees_manager.has_protocol_unsettled_fee<DEEP>(), true);
        assert_eq!(fees_manager.get_protocol_unsettled_fee_balance<SUI>(), sui_fee_amount);
        assert_eq!(fees_manager.get_protocol_unsettled_fee_balance<DEEP>(), deep_fee_amount);
...
#[test, expected_failure(abort_code = deeptrade_core::fees_manager::EInvalidOwner)]
