#[test_only]
module deeptrade_core::prepare_input_fee_order_execution_tests;
...
fun bid_success() {
    ...
    // Step 1: Setup deepbook infrastructure
    ...
    // Step 2: Create pool with default fees (SUI/USDC)
    ...
    scenario.next_tx(ALICE);
    {
        ...
        // Execute prepare_input_fee_order_execution
        let trade_proof = prepare_input_fee_order_execution(
            &pool,
            &mut balance_manager,
            &mut base_coin_mut,
            &mut quote_coin_mut,
...
fun invalid_owner_aborts() {
    ...
    // Step 4: Execute the test from BOB (not the owner) - should abort
